...
import { Menu, Plus, Trash2, Folder, Keyboard } from 'lucide-react';
...
export const ProjectManagementScreen: React.FC<ProjectManagementProps> = ({ onOpenDrawer }) => {
  const { projects, addProject, deleteProject, timeEntries } = useTimeEntries();
  ...
  const handleDelete = (id: string, name: string) => {
    // Check if in use
    const inUse = timeEntries.some((e) => e.projectId === id);
    if (inUse) {
    ...
    <div className="flex flex-col h-full bg-[#F3F4F6] text-slate-900 relative">
      ...
      <header className="sticky top-0 bg-white border-b border-slate-200 text-slate-900 shadow-none z-10 px-5 py-3.5 flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-base font-extrabold tracking-tight">Projects</h1>
        ...
        {projects.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl flex flex-col items-center justify-center py-16 px-6 text-neutral-400 text-center shadow-xs">
            <div className="w-16 h-16 bg-slate-50 rounded-full flex items-center justify-center mb-4 text-slate-400 border border-slate-100">
              <Folder className="w-8 h-8 text-indigo-505" />
            </div>
            <p className="font-semibold text-slate-700">No projects yet!</p>
            <p className="text-xs mt-1 text-slate-400">Tap the button above to add your first project folder.</p>
          </div>
        ) : (
          <div className="bg-white rounded-xl shadow-xs border border-slate-200 divide-y divide-slate-100 overflow-hidden">
            <div className="px-4 py-3 bg-slate-150/50 border-b border-slate-150 flex justify-between items-center bg-slate-50">
              <h3 className="text-[10px] font-bold uppercase text-slate-500 tracking-wider">Project Folder Stack</h3>
            ...
            <AnimatePresence initial={false}>
              {projects.map((project, index) => {
                ...
                const usageCount = timeEntries.filter((e) => e.projectId === project.id).length;
                return (
                  <motion.div
                  ...
                  >
                    <div className="flex items-center space-x-3.5">
                      ...
                      <div>
                        <p className="font-bold text-xs text-slate-800">{project.name}</p>
            ...
            <div className="fixed inset-0 flex items-center justify-center z-40 p-4 pointer-events-none">
              ...
              >
                <div className="flex items-center space-x-2.5 mb-3.5">
                  ...
                  <h3 className="font-bold text-sm text-slate-800">Add Project</h3>
                </div>
                ...
                <div className="space-y-3.5">
                  <div>
                    ...
                    />
                    {errorText && (
                      <p id="error-project" className="text-[10px] text-red-500 mt-1.5 font-semibold">
                        {errorText}
                      </p>
                    )}
                  </div>
